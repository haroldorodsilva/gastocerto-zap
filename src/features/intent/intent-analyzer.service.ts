...
import {
  ...
  TIME_INDICATORS,
  ABBREVIATION_MAP,
  GENERATE_CHART_KEYWORDS,
} from './intent-keywords';
import { normalizeForIntent, normalizeKeyword, fuzzyMatchKeyword } from '@shared/utils/string-utils';
import {
  getHumanGreeting,
  getHowAreYouReply,
  getUnknownMessage,
  getHelpMessage as getHumanHelpMessage,
...
export enum MessageIntent {
  ...
  GENERATE_CHART = 'GENERATE_CHART', // Gerar gráfico
...
export class IntentAnalyzerService {
   ...
   * - lowercase + remoção de acentos + expansão de abreviações
   */
  private normalizeText(text: string): string {
    let normalized = normalizeForIntent(text);

    // Expandir abreviações conhecidas (palavra inteira)
  ...
  /**
   * Verifica se o texto normalizado contém alguma keyword (também normalizada)
   */
  ...
  /**
   * Verifica correspondência exata ou início de frase contra keywords normalizadas
   */
  private exactMatchAny(text: string, keywords: readonly string[]): boolean {
    return keywords.some((k) => {
      const nk = normalizeForIntent(k);
      return text === nk || text.startsWith(nk + ' ');
    });
  }
  ...
  async analyzeIntent(
    ...
    // 8.5. Verificar pedido de gráfico (antes de categorias para evitar conflito)
    if (this.matchesAny(normalizedText, GENERATE_CHART_KEYWORDS)) {
      this.logger.log(`✅ Intent: GENERATE_CHART (confidence: 0.95)`);
      const isIncome = normalizedText.includes('receita') || normalizedText.includes('ganho');
      return {
        intent: MessageIntent.GENERATE_CHART,
        confidence: 0.95,
        shouldProcess: true,
        metadata: {
          chartType: isIncome ? 'income' : 'categories',
    ...
    // 11.1. Tentar fuzzy matching como última tentativa (só textos curtos)
    const fuzzyResult = this.tryFuzzyIntentMatch(normalizedText);
    if (fuzzyResult) {
      this.logger.log(`🔍 Fuzzy match found: ${fuzzyResult.intent} (via "${fuzzyResult.matchedKeyword}")`);
      return fuzzyResult.result;
    }

    // 11.2. Tentar desambiguação com opções antes de devolver UNKNOWN
    const disambiguationMsg = await this.disambiguation.suggest(normalizedText, phoneNumber);

    return {
      intent: MessageIntent.UNKNOWN,
      confidence: transactionAnalysis.confidence,
      shouldProcess: false,
      suggestedResponse: disambiguationMsg || getUnknownMessage(),
    };
  }
  ...
  private analyzeTransactionIntent(text: string): {
    ...
    for (const verb of TRANSACTION_VERBS) {
      if (text.includes(normalizeForIntent(verb))) {
    ...
    for (const keyword of CATEGORY_KEYWORDS) {
      if (text.includes(normalizeForIntent(keyword))) {
    ...
    for (const time of TIME_INDICATORS) {
      if (text.includes(normalizeForIntent(time))) {
  ...
  private isGreeting(text: string): boolean {
    return this.exactMatchAny(text, GREETING_KEYWORDS);
  }
  ...
  private getContextualGreeting(text: string): string {
    const isAskingHowAreYou = this.matchesAny(text, HOW_ARE_YOU_KEYWORDS);

    let greeting = getHumanGreeting();
    ...
    greeting +=
      ...
      '💸 _"Gastei 50 no mercado"_ — Registrar gastos\n' +
      '📊 _"Resumo do mês"_ ou _"Meu saldo"_\n' +
      '📋 _"Minhas transações"_ — Listar\n' +
      '📂 _"Gastos por categoria"_\n' +
      '📷 Envie foto de nota fiscal\n' +
      '🎤 Grave um áudio\n\n' +
      'Use linguagem natural, tô aqui pra ajudar! 😊';
  ...
  private isMonthlySummaryRequest(text: string): boolean {
    return this.matchesAny(text, MONTHLY_SUMMARY_KEYWORDS);
  }
  ...
  private isCategoryBreakdownRequest(text: string): boolean {
    return this.matchesAny(text, CATEGORY_BREAKDOWN_KEYWORDS);
  }
  ...
  private isHelpRequest(text: string): boolean {
    return this.matchesAny(text, HELP_KEYWORDS);
  }
  ...
  private isBalanceCheck(text: string): boolean {
    return this.matchesAny(text, BALANCE_KEYWORDS);
  }
  ...
  private isListTransactions(text: string): boolean {
    return this.matchesAny(text, LIST_TRANSACTIONS_KEYWORDS);
  }
  ...
  private isConfirmationResponse(text: string): boolean {
    ...
    return (
      this.exactMatchAny(text, YES_RESPONSES) ||
      this.exactMatchAny(text, NO_RESPONSES)
    );
  }
  ...
  private isListPendingRequest(text: string): boolean {
    return this.matchesAny(text, LIST_PENDING_CONFIRMATION_KEYWORDS);
  }
  ...
  private isListPendingPaymentsRequest(text: string): boolean {
    ...
    const hasPendingWord = this.exactMatchAny(text, PENDING_STANDALONE_WORDS);

    return this.matchesAny(text, LIST_PENDING_PAYMENT_KEYWORDS) || hasPendingWord;
  }
  ...
  private isSwitchAccountRequest(text: string): boolean {
    ...
    if (this.matchesAny(text, SWITCH_ACCOUNT_KEYWORDS)) {
      return true;
    }
  ...
  private isListAccountsRequest(text: string): boolean {
    return this.matchesAny(text, LIST_ACCOUNTS_KEYWORDS);
  }
  ...
  private isShowActiveAccountRequest(text: string): boolean {
    return this.matchesAny(text, SHOW_ACTIVE_ACCOUNT_KEYWORDS);
  }
  ...
  private isPayBillRequest(text: string): boolean {
    return this.matchesAny(text, PAY_BILL_KEYWORDS);
  }
  ...
  private isListCreditCardsRequest(text: string): boolean {
    return this.matchesAny(text, LIST_CREDIT_CARDS_KEYWORDS);
  }
  ...
  private isSetDefaultCreditCardRequest(text: string): boolean {
    return this.matchesAny(text, SET_DEFAULT_CARD_KEYWORDS);
  }
  ...
  private isShowDefaultCreditCardRequest(text: string): boolean {
    ...
    if (this.matchesAny(text, SHOW_DEFAULT_CARD_EXCLUSIONS)) {
      return false;
    }
    return this.matchesAny(text, SHOW_DEFAULT_CARD_KEYWORDS);
  }
  ...
  private isShowInvoiceByCardNameRequest(text: string): boolean {
    const hasInvoiceKeyword = this.matchesAny(text, INVOICE_TRIGGER_KEYWORDS);

    // Verificar se não é comando de lista genérica
    const isGenericList =
      this.matchesAny(text, INVOICE_GENERIC_LIST_KEYWORDS) ||
  ...
  private isListInvoicesRequest(text: string): boolean {
    return this.matchesAny(text, LIST_INVOICES_KEYWORDS);
  }
  ...
  private isShowInvoiceDetailsRequest(text: string): boolean {
    return this.matchesAny(text, INVOICE_DETAILS_KEYWORDS);
  }
  ...
  private isPayInvoiceRequest(text: string): boolean {
    return this.matchesAny(text, PAY_INVOICE_KEYWORDS);
  }
  ...
  /**
   * Tenta encontrar intenção via fuzzy matching (Levenshtein distance).
   * Só para textos curtos (até 3 palavras) — evita falsos positivos.
   */
  private tryFuzzyIntentMatch(
    text: string,
  ): { intent: MessageIntent; matchedKeyword: string; result: IntentAnalysisResult } | null {
    const words = text.trim().split(/\s+/);
    ...
    const keywordGroups: Array<{
      ...
      { keywords: LIST_TRANSACTIONS_KEYWORDS, intent: MessageIntent.LIST_TRANSACTIONS, confidence: 0.7, shouldProcess: true },
      { keywords: BALANCE_KEYWORDS, intent: MessageIntent.CHECK_BALANCE, confidence: 0.7, shouldProcess: true },
      { keywords: MONTHLY_SUMMARY_KEYWORDS, intent: MessageIntent.MONTHLY_SUMMARY, confidence: 0.7, shouldProcess: true },
      { keywords: CATEGORY_BREAKDOWN_KEYWORDS, intent: MessageIntent.CATEGORY_BREAKDOWN, confidence: 0.7, shouldProcess: true },
      { keywords: LIST_CREDIT_CARDS_KEYWORDS, intent: MessageIntent.LIST_CREDIT_CARDS, confidence: 0.7, shouldProcess: true },
      { keywords: LIST_INVOICES_KEYWORDS, intent: MessageIntent.LIST_INVOICES, confidence: 0.7, shouldProcess: true },
      { keywords: GENERATE_CHART_KEYWORDS, intent: MessageIntent.GENERATE_CHART, confidence: 0.7, shouldProcess: true },
      { keywords: HELP_KEYWORDS, intent: MessageIntent.HELP, confidence: 0.7, shouldProcess: false },
    ];

    for (const group of keywordGroups) {
      const normalizedKeywords = group.keywords.map((k) => normalizeForIntent(k));
      const match = fuzzyMatchKeyword(text, normalizedKeywords, 0.75);
      if (match) {
        return {
          intent: group.intent,
          ...
          result: {
            ...
            suggestedResponse: group.intent === MessageIntent.HELP ? this.getHelpMessage() : undefined,
          },
        };
      }
    }
